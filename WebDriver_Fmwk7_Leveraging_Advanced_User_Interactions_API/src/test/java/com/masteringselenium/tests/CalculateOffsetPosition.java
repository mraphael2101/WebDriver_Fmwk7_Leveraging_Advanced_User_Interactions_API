...
/* this class works out where the cursor needs to be placed based on the following assumptions
...
{
    public enum CursorPosition { TOP_LEFT, CENTER }

    final WebElement parentElement, childElement;
    ...
    {
        this.parentElement = parentElement; this.childElement = childElement; this.cursorPosition = cursorPosition;
        calculateOffset();
    }
    ...
    private void calculateOffset() throws ElementNotVisibleException 
    {
        ...
        if (cursorPosition.equals(TOP_LEFT)) { xOffset = 1; yOffset = 1; }

        if (cursorPosition.equals(CENTER)) 
        {
            if (elementTwoWidth  < elementOneWidth ) { xOffset = (elementTwoWidth  / 2) + 1;  }
            if (elementTwoHeight < elementOneHeight) { yOffset = (elementTwoHeight / 2) + 1;  }
        }
    }
}